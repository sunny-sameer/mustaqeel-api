...
class StageStatusUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'name' => [
                ...
                'max:50',
                'regex:/^[a-zA-Z.,، ]+$/u',
                Rule::unique('stages_statuses', 'name')->ignore($id)
                    ->where(fn ($q) => $q->where('stageId', $stageId)),
            ],

            'nameAr' => self::arabicNameRule(
                Rule::unique('stages_statuses', 'nameAr')->ignore($id)
                    ->where(fn ($q) => $q->where('stageId', $stageId))
            ),
